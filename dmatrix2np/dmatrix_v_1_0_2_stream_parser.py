...
class DMatrixStreamParserV1_0_2(DMatrixStreamParser):
    ...
    def _parse_version(self):
        verstr = self._handle.read(8)
        if verstr != self.verstr.encode():
            raise InvalidStructure('Invalid verstr')
        self._ver_major = int.from_bytes(self._handle.read(4), byte_order_str)
        self._ver_minor = int.from_bytes(self._handle.read(4), byte_order_str)
        self._ver_patch = int.from_bytes(self._handle.read(4), byte_order_str)
